...
import { ACTION_LABELS, STATUS_LABELS, downloadCsv, formatCoins, formatDate, orderTitle } from './presentation';
...
export function OrdersTab({ orders, refresh }: { orders: Order[]; refresh: () => Promise<void> }) {
  ...
  const filtered = orders.filter((order) => `${orderTitle(order)} ${order.customerName} ${order.customerPhone} ${order.id} ${order.pickupCode || ''}`.toLowerCase().includes(search.toLowerCase())
    && (status === 'all' || order.status === status || (status === 'action' && Boolean(order.nextStatus))));
  ...
  return <div className="page-stack">
    <div className="section-actions"><span className="muted">Показано {filtered.length} из {orders.length} заказов</span><button className="button button--primary" onClick={() => downloadCsv('shop-orders.csv', [
      ['Заказ', 'Дата', 'Клиент', 'Телефон', 'Товары', 'Статус', 'MSI Coin', 'Код выдачи'],
      ...filtered.map((order) => [order.id, formatDate(order.createdAt), order.customerName, order.customerPhone, orderTitle(order), STATUS_LABELS[order.status || 'paid'], order.totalPrice, order.pickupCode || '']),
    ])}>↓ Экспорт CSV</button></div>
    {error ? <p className="inline-error" role="alert">{error}</p> : null}
    <div className="panel table-toolbar"><label className="search-field"><SearchIcon /><input aria-label="Поиск заказов" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Клиент, товар, номер или код выдачи…" /></label><label className="filter-field"><span>Статус</span><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">Все статусы</option><option value="action">Требуют действия</option>{Object.entries(STATUS_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div>
    <section className="panel"><div className="table-scroll"><table className="responsive-table orders-table"><thead><tr><th>Дата</th><th>Клиент</th><th>Товары</th><th>Статус</th><th className="numeric">Сумма</th><th>Действия</th></tr></thead><tbody>{filtered.map((order) => <tr key={order.id}>
      ...
      <td data-label="Товары">{orderTitle(order)}{order.pickupCode ? <small className="cell-subline pickup-code">Код: {order.pickupCode}</small> : null}{order.pickupSlot ? <small className="cell-subline">{order.pickupSlot}</small> : null}</td>
      <td data-label="Статус"><span className={`order-status order-status--${order.status}`}>{STATUS_LABELS[order.status || 'paid']}</span></td>
      <td data-label="Сумма" className="numeric">Ⓒ {formatCoins(order.totalPrice)}</td>
      <td className="actions-cell">{order.nextStatus ? <button className="button button--small button--primary" disabled={Boolean(busyId)} onClick={() => void advance(order)}>{busyId === order.id ? 'Сохраняем…' : ACTION_LABELS[order.nextStatus as OrderStatus]}</button> : <span className="done-label">✓ Завершён</span>}</td>
